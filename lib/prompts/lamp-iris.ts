...
/**
 * Immutable task contract for the eye-contact experiment. The edit surface is
 * the smallest of any Lamp mode — gaze direction and the eyelid pose it
 * implies — so the locks enumerate everything else, and the characteristic
 * overshoot (a dead stare) is named a failure in the task itself.
 */
 ...
 * (LAMP-INTENSITY.md). Each band states what must be TRUE of the gaze at that
 * level; the keep-clause states what survives at every level. For this mode
 * the ladder runs between two failure modes: still-reading (undershoot) and
 * dead stare (overshoot).
 */
 ...
 * approved plan. Lamp Iris has a single prompt generation so far; if the base
 * contract is ever rewritten, freeze the old form here the way Lamp Beautify
 * freezes its legacy generations.
 */
export function isPersistedInitialLampIrisPrompt(
  plan: LampIrisPlan,
  rendered: string
): boolean {
  return rendered === initialLampIrisMegaPrompt(plan).rendered;
}
...
function renderPersistedV2(
  ...
  if (persistedPlanBlock !== renderLampIrisPlanBlock(plan)) {
    throw new Error(
