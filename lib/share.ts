...
export async function buildShareSnapshot(run: Run): Promise<string> {
  ...
  verdictBits.push(
    lamp
      ? `${attempts}/2 videos · Final is v2`
      : `${attempts} attempt${attempts === 1 ? "" : "s"}`
  );
  if (!lamp && run.bestIterationIndex !== undefined) {
  ...
  const summaryData = {
    ...
    compositeLine: composite
      ? `Composite: ${Math.round(composite.score)}/100 — ${composite.passed ? "PASSED" : "DID NOT PASS"} · ${lamp ? `${attempts}/2 videos · Final is v2` : `${attempts} attempt${attempts === 1 ? "" : "s"}${run.bestIterationIndex !== undefined ? ` · best iteration ${run.bestIterationIndex}` : ""}`}`
      : `Composite: — (no completed iteration yet) · ${lamp ? `${attempts}/2 videos` : `${attempts} attempt${attempts === 1 ? "" : "s"}`}`,
    ...
    footer: `Generated by Lamp · mock simulation · ${generatedAt}`,
  };
...
${rows.map((r) => r.html).join("\n")}
  ...
  <footer>Generated by Lamp · mock simulation · ${esc(generatedAt)}</footer>
</div>
...
(function () {
  ...
  copyBtn.addEventListener('click', function () {
    var lines = [];
    lines.push('Lamp — review feedback');
