...
import { isLampBeautifyEvaluationArtifact } from "@/lib/lamp-beautify-read";
import {
...
const VIOLATION_SCHEMA = {
  ...
  properties: {
    ...
    correctionAction: {
      type: ["string", "null"],
      enum: [...LAMP_BEAUTIFY_CORRECTION_ACTIONS, null],
    },
...
const RESULT_SCHEMA = {
  ...
  properties: {
    evalId: {
      type: "string",
      enum: LAMP_BEAUTIFY_VISUAL_EVAL_DEFS.map((definition) => definition.id),
    },
    score: { type: "number" },
...
async function generateWithRateLimitRetry<T>(
  ...
  for (const delayMs of RATE_LIMIT_RETRY_DELAYS_MS) {
    try {
      ...
      console.warn(
        `[lamp-beautify-evaluator] rate limit rejected the judge call; retrying in ${Math.round(
          delayMs / 1000
        )}s`
      );
...
export async function runLampBeautifyHolisticEvaluation(input: {
  runId: string;
  iteration: 1 | 2;
  ...
  if (plan.approval.status !== "approved") {
    throw new Error(
      "Lamp Beautify evaluation requires the approved enhancement plan."
    );
  }
  const storage = getStorage();
  const run = await storage.getRun(input.runId);
  if (!run) {
    throw new Error("Run not found for Lamp Beautify evaluation.");
  }
  ...
  if (
    ...
    throw new Error(
      "Lamp Beautify evaluation requires a completed, prompt-bound generation with verified original audio."
    );
  }

  const prompt = renderLampBeautifyHolisticEvaluatorPrompt({
    plan,
    iteration: input.iteration,
  });
  const operationId = lampBeautifyEvaluationOperationId(input.iteration);
  ...
  if (claim.state === "cached") {
    ...
    if (!isLampBeautifyEvaluationArtifact(cached, input.iteration)) {
      throw new Error(
        "Cached Lamp Beautify evaluation has an invalid artifact."
      );
    }
    return cached;
  }
  ...
  try {
    ...
    if (!response.text) {
      throw new Error("Lamp Beautify evaluator returned no content.");
    }
    ...
    await markPaidOperationReconcileRequired(
      ...
      error instanceof Error
        ? error.message
        : "Lamp Beautify evaluation returned an ambiguous result."
    );
    throw error;
  }
}
